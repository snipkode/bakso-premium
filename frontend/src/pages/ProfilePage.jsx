  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-orange-50/50 to-background pb-24">
      ...
      <div className="px-4 py-4">
        <Card className="p-5 bg-gradient-to-br from-primary to-orange-500 text-white relative overflow-hidden shadow-xl">
          {/* Background Pattern */}
          ...
          <div className="relative z-10">
            ...
            <div className="flex items-start gap-3 mb-4">
              {/* Avatar */}
              <div className="w-16 h-16 rounded-full bg-white/20 backdrop-blur-sm flex items-center justify-center text-2xl font-bold border-2 border-white/30 shadow-lg relative flex-shrink-0">
                {user?.name?.charAt(0).toUpperCase()}
                {/* Verified Badge */}
                {user?.email ? (
                  <div className="absolute -bottom-0.5 -right-0.5 w-6 h-6 bg-green-500 rounded-full border-2 border-primary flex items-center justify-center shadow-lg" title="Akun Terverifikasi">
                    <svg className="w-3 h-3 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="3" d="M5 13l4 4L19 7" />
                    </svg>
                  </div>
                ) : (
                  <div className="absolute -bottom-0.5 -right-0.5 w-6 h-6 bg-red-500 rounded-full border-2 border-primary flex items-center justify-center shadow-lg" title="Belum Terverifikasi">
                    <svg className="w-3 h-3 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-start justify-between gap-2 mb-1">
                  <div className="flex-1 min-w-0">
                    <h2 className="text-lg font-bold truncate flex items-center gap-1.5">
                      {user?.name || 'User'}
                      {user?.email && (
                        <span className="text-[10px] bg-white/20 backdrop-blur-sm px-1.5 py-0.5 rounded-full flex items-center gap-1 flex-shrink-0">
                  ...
                  <IconButton
                    onClick={handleEditClick}
                    className="bg-white/20 hover:bg-white/30 text-white flex-shrink-0 w-8 h-8"
                  >
                ...
                <div className="space-y-1">
                  <p className="text-xs text-white/90 flex items-center gap-1.5 truncate">
                    ...
                    <span className="truncate">{user?.phone || 'Belum diatur'}</span>
                  </p>
                  <div className="text-xs text-white/90 flex items-center gap-1.5 truncate">
                    <Mail className="w-3 h-3 flex-shrink-0" />
                    {user?.email ? (
                      <div className="flex items-center gap-1.5 truncate flex-1 min-w-0">
                        <span className="truncate">{user.email}</span>
                        {user.email_verified ? (
                          <span className="text-[10px] text-green-300 flex items-center gap-1 flex-shrink-0">
                          ...
                          <button
                            ...
                            className="text-[10px] bg-white/20 hover:bg-white/30 disabled:bg-white/10 px-1.5 py-0.5 rounded-full flex items-center gap-1 transition-all flex-shrink-0"
                          >
                            {sendingVerification ? (
                              <>
              ...
              <div className="bg-white/10 backdrop-blur-sm rounded-lg p-2.5">
                <div className="flex items-center justify-between mb-1">
                  <div className="flex items-center gap-1.5">
                    ...
                    <span className="text-[10px] text-white/80 font-medium">Total Pesanan</span>
                  </div>
                  <button
                    onClick={fetchStats}
              ...
              <div className="bg-white/10 backdrop-blur-sm rounded-lg p-2.5">
                <div className="flex items-center justify-between mb-1">
                  <div className="flex items-center gap-1.5">
                    ...
                    <span className="text-[10px] text-white/80 font-medium">Poin Loyalty</span>
                  </div>
                  <button
                    onClick={fetchStats}
                    className="text-white/60 hover:text-white transition-colors"
                ...
                <p className="text-xl font-bold">{loading ? '-' : stats.loyaltyPoints}</p>
              </div>
            </div>
          </div>
        </Card>
